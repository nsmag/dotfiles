...
const QuestionOptionSchema = Type.Object({
  ...
  description: Type.Optional(
    Type.String({
      description: "Optional clarifying description shown below the label",
    }),
  ),
});
...
const QuestionSchema = Type.Object({
  id: Type.String({
    description: "Unique identifier for this question (used in the answer map)",
  }),
  label: Type.Optional(
    Type.String({
      description:
  ...
  options: Type.Array(QuestionOptionSchema, {
    description: "Options the user may choose from",
  }),
  allowOther: Type.Optional(
    Type.Boolean({
      description: "Allow a free-text 'Type something' option (default: true)",
    }),
  ),
});
...
const QuestionnaireParams = Type.Object({
  questions: Type.Array(QuestionSchema, {
    description:
      "One or more questions to present. Use multiple questions in a single call rather than calling this tool repeatedly.",
  }),
});
  ...
  pi.registerTool({
              ...
              for (let i = 0; i < opts.length; i++) {
                ...
                if (isOther && inputMode) {
                  ...
                  for (let li = 0; li < labelLines.length; li++) {
                    const linePrefix =
                      li === 0 ? theme.fg(color, prefix) : labelIndent;
                    lines.push(
                      truncateToWidth(linePrefix + labelLines[li], width),
                    );
                  }
                }
                if (opt.description) {
    ...
    renderResult(result, _options, theme, _context) {
      ...
      const lines = details.answers.map((a) => {
        const prefix =
          theme.fg("success", "✓ ") + theme.fg("accent", `${a.id}: `);
        if (a.wasCustom) {
